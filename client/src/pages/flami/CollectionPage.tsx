      ...
      <div className="grid grid-cols-3 gap-6 w-full text-xs">
        {collection.length === 0 ? (
          <span className="col-span-3 text-xl">
            Tu n'as pas encore de Flamis dans ta collection !
          </span>
        ) : (
        ...
        {[...Array(9 - collection.length).keys()].map((i) => (
          <img
            ...
            className={`relative z-10 w-full scale-65 translate-y-2 max-h-60 grayscale ${
              i > 3 ? `opacity-${30 - (i - 3) * 5}` : "opacity-30"
            }`}
            alt="Flami"
          />
        ))}
