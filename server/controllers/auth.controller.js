...
const authController = {
  signup: async (req, res) => {
    ...
    try {
      ...
      if (error.keyValue && error.keyValue.email) {
        return res.status(409).json({ message: `Un compte avec cet email (${error.keyValue.email}) existe déjà.`, error: 409 })
      } else if (error.keyValue && error.keyValue.name) {
        return res.status(409).json({ message: `Un compte avec ce nom (${error.keyValue.name}) existe déjà.`, error: 409 })
      } else if (error.keyValue && error.keyValue.age) {
        return res.status(409).json({ message: `L'âge requis pour utiliser cette application est de 13 ans.`, error: 409 })
      } else {
  ...
  signin: async (req, res) => {
    ...
    try {
      ...
      if(!user[0]) {
        return res.status(404).json({ message: "Ce compte n'existe pas.", error: 404 });
      }
